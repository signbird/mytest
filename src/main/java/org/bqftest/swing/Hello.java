...
滑块也可以在固定时间间隔（或在任意位置）沿滑块刻度打印文本标签。标签的绘制由 setLabelTable 和 setPaintLabels 控制
 http://www.apihome.cn/api/java/JSlider.html
	 */
	private JSlider slider;
	
	/**
	 * 让用户从一个有序序列中选择一个数字或者一个对象值的单行输入字段。
	 * Spinner 通常提供一对带小箭头的按钮以便逐步遍历序列元素。
	 * 键盘的向上/向下方向键也可循环遍历元素。也允许用户在 spinner 中直接输入合法值。
	 * 尽管组合框提供了相似的功能，但因为 spinner 不要求可隐藏重要数据的下拉列表，所以有时它也成为首要选择。 
	 * 
	 * http://www.apihome.cn/api/java/jspinner.html
	 */
